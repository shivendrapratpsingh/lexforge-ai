...
async function getDashboardData(userId) {
  try {
    ...
    const [drafts, total, finalized] = await Promise.all([
    ...
    return { drafts, total, finalized, error: null }
  } catch (err) {
    console.error('[Dashboard] DB error:', err)
    return { drafts: [], total: 0, finalized: 0, error: 'Database not connected. Run: npx prisma db push' }
  }
}
...
export default async function DashboardPage() {
  ...
  const { drafts, total, finalized, error } = await getDashboardData(session.user.id)
  ...
  const stats = [
    ...
    { label: 'Drafts', value: total - finalized, icon: '✏️', color: '#F59E0B' },
    { label: 'Doc Types', value: 5, icon: '⚖️', color: '#8B5CF6' },
  ]
          ...
          <div style={{ background: '#141414', border: '1px solid #2A2A2A', borderRadius: 16, padding: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <Link href="/drafts" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', background: '#1C1C1C', border: '1px solid #2A2A2A', borderRadius: 10, textDecoration: 'none', color: '#A0A0A0', fontSize: 14 }}>
                <span style={{ color: '#D4A017' }}>◉</span> View All Drafts
              </Link>
            </div>
          </div>
