...
// The full per-type schema now lives in lib/document-fields.js so this
// intake form and the server-side extractor (lib/groq.js) share ONE
// source of truth across all 20 document services. FIELDS is kept as a
// local alias so the rest of this file is unchanged.
const FIELDS = DOC_FIELDS

// Per-service hint for the "Upload Folder / File" intake — tells the
// user, for the document type they picked, exactly what the AI will do
// with their files and which fields it will try to auto-fill.
function folderHintFor(type) {
  if (!type) return undefined
  if (type === 'CASE_BRIEF') {
    return 'AI reads every file in the folder — or a single Word doc / scanned PDF — and produces a short, executive-style case brief: parties, chronology, issues, strengths, weaknesses and next steps.'
  }
  const doc        = DOCUMENT_TYPES.find(d => d.value === type)
  const label      = doc ? doc.label : 'document'
  const defs       = FIELDS[type] || []
  const fieldNames = defs.slice(0, 4).map(f => f.label.replace(/\s*\(.*?\)\s*/g, '').trim()).join(', ')
  return `AI reads every file in the folder — or a single Word doc / scanned PDF — and auto-fills the ${label} form${fieldNames ? ` (${fieldNames}…)` : ''} from their contents. Review and edit before generating.`
}
  ...
  async function handleFolderExtract() {
    ...
    try {
      ...
      if (data?.extracted && Object.keys(data.extracted).length > 0) {
        ...
        // Empty result = the AI found nothing in the file that matches the
        // fields THIS document type needs. Usually means the uploaded file
        // is unrelated to the document type the user picked (we never
        // invent details, so an off-topic file yields nothing).
        const docLabel   = DOCUMENT_TYPES.find(d => d.value === selectedType)?.label || 'this document'
        const wantFields = (FIELDS[selectedType] || [])
          .slice(0, 3)
          .map(f => f.label.replace(/\s*\(.*?\)\s*/g, '').trim())
          .join(', ')
        setError(
          `The uploaded file doesn't appear to contain ${docLabel} details` +
          (wantFields ? ` — nothing matching "${wantFields}…" was found in it. ` : '. ') +
          `Upload a file that actually relates to a ${docLabel}, or pick the document type that matches your file. You can also fill the form manually.`
        )
      }
    } catch (e) {
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-4 lg:gap-[18px] items-start">
          ...
          <div style={{ background: '#141414', border: '1px solid #2A2A2A', borderRadius: 16, overflow: 'hidden' }}>
            ...
            {/* Copy to clipboard — works for every one of the 20 services */}
            ...
            <div style={S.card}>
              {!extracted && !shortBrief ? (
                <div>
                  ...
                  <FolderUploader
                    hint={folderHintFor(selectedType)}
