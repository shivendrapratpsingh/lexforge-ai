...
// Flatten the analyser's structured result into something worth
// printing. Order follows what a lawyer does with an order: what was
// directed, what must happen now, by when, and where it hurts.
const NL = '\n'
function orderAnalysisText(r) {
  if (!r) return ''
  const line = (i) => `- ${typeof i === 'string' ? i : (i.task || JSON.stringify(i))}`
  const block = (heading, items) =>
    Array.isArray(items) && items.length
      ? [heading.toUpperCase(), ...items.map(line), ''].join(NL)
      : ''
  const dates = Array.isArray(r.complianceDates) && r.complianceDates.length
    ? ['COMPLIANCE DATES', ...r.complianceDates.map(d =>
        typeof d === 'string' ? `- ${d}` : `- ${d.task || ''}${d.date ? ` — ${d.date}` : ''}`), ''].join(NL)
    : ''
  return [
    'ORDER ANALYSIS',
    r.orderType ? `Type: ${r.orderType}` : '',
    r.nextDate ? `Next date: ${r.nextDate}` : '',
    '',
    block('Court directions', r.directions),
    block('Immediate actions', r.immediateActions),
    block('Documents needed', r.documentsNeeded),
    dates,
    block('Favourable points', r.favorablePoints),
    block('Adverse points', r.adversePoints),
    '',
    'Generated by LexForge AI. Verify every direction against the order itself.',
      ...
      {result && (
        <div style={{ marginTop: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#D4A017', letterSpacing: '1.2px', textTransform: 'uppercase' }}>
              Order Analysis
            </div>
            {/* This result is a set of lists rather than a block of prose,
                so it is flattened at click time into the same shape a
                lawyer would write it out in. */}
            <DownloadButtons compact title="Order Analysis" content={() => orderAnalysisText(result)} />
          </div>
          {/* Order Type */}
          {result.orderType && (
    ...
    <div style={{ marginTop: 20 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <DownloadButtons title={title} content={content} compact />
          {draft?.id && (
            <>
