...
import { buildCaseBriefMessages, isRefusal } from './groq.js'  // re-uses prompt builder
...
// Tier-aware sizing — kept independent of app-tier so admin/Pro users on
// a free-Groq key still send small requests that fit the TPM budget.
const groqPaid     = process.env.GROQ_PAID_TIER === '1'
const WINDOW_CHARS = groqPaid ? 12_000 : 4_000
const PER_TICK_MAX = groqPaid ? 5      : 2      // extractions per worker invocation
const PACE_MS      = groqPaid ? 1_500  : 4_000  // pause between Groq calls inside one tick
const EXTRACT_MAX_TOKENS  = groqPaid ? 600   : 280
const SYNTH_MAX_TOKENS    = groqPaid ? 4_000 : 1_200
const SYNTH_HEAD          = groqPaid ? 30_000 : 6_000  // chars of ledger we send to synth

const MODEL_LIST = groqPaid
