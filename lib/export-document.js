...
export async function buildPdf({ content }) {
  ...
  // jsPDF's built-in `times` is a standard-14 font: single-byte,
  // WinAnsi. It silently corrupted every ₹ amount, deleted every em
  // dash, printed '%' for each '─' and destroyed every Indic script.
  // Embed real Unicode faces for whatever this document actually uses,
  // and only fall back to `times` — with the lossy sanitiser — if none
  // of them could be loaded. See lib/pdf-fonts.js.
  const { registerFonts, familyForLine, normaliseForPdf, sanitiseForWinAnsi } =
    await import('./pdf-fonts.js')
  // normaliseForPdf runs unconditionally: box-drawing rules exist in no
  // text font, so once the Unicode face was embedded they stopped
  // printing as '%' and started disappearing altogether.
  let body = normaliseForPdf(content)
  const loaded = await registerFonts(doc, body)
  if (!loaded.latin) body = sanitiseForWinAnsi(body)
  const latinFamily = loaded.latin || 'times'
  ...
  for (const rawLine of body.split('\n')) {
    ...
    // Per line, because a document can mix scripts — an English cause
    // title above a Hindi prayer. One font per jsPDF text call.
    const family = familyForLine(trimmed, loaded) || latinFamily

    doc.setFontSize(14)
    doc.setFont(family, heading ? 'bold' : 'normal')
  ...
  for (let i = 1; i <= pageCount; i++) {
    doc.setPage(i)
    doc.setFontSize(11)
    doc.setFont(latinFamily, 'normal')
