...
import { buildLexiconBlock, TYPE_GROUP } from './legal-dictionary.js'
...
═══════ LAYOUT & PRESENTATION RULES (INDIAN COURT FILING FORMAT) ═══════
These rules govern the PHYSICAL LAYOUT of the text so the exported PDF/DOCX
can be filed directly. Lines containing ONLY a court name, ONLY the word
VERSUS, or ONLY an ALL-CAPS heading are automatically centred by the export
engine — keep such lines free of any other text.

A. COURT FILINGS (petitions, applications, affidavits, complaints filed in a court/forum):
   1. Line 1: the full court name in capitals, alone on its line ("IN THE HIGH COURT OF ...").
   2. Then the jurisdiction/side where applicable ("CIVIL APPELLATE JURISDICTION") and the
      case-number block ("WRIT PETITION (CIVIL) NO. _____ OF 20__") each on its own line.
   3. Then the parties block:
        [FULL NAME AND DESCRIPTION OF PARTY]        ...PETITIONER/APPLICANT/COMPLAINANT
        VERSUS
        [FULL NAME AND DESCRIPTION OF PARTY]        ...RESPONDENT/OPPOSITE PARTY
      "VERSUS" is always written in full, alone on its own line.
   4. Then the document title in capitals on its own line, stating the filing and its
      enabling provision ("APPLICATION UNDER SECTION 439 OF THE CODE OF CRIMINAL PROCEDURE, 1973 ...").
   5. Where the document type uses it, the address block: "TO," / "THE HON'BLE CHIEF JUSTICE
      AND HIS COMPANION JUSTICES OF ..." followed by "THE HUMBLE PETITION OF THE PETITIONER
      ABOVENAMED" and "MOST RESPECTFULLY SHOWETH:".
   6. Facts and grounds in numbered paragraphs — "1. That ..." — one point per paragraph;
      sub-points as (a), (b), (c).
   7. PRAYER as lettered reliefs (a), (b), (c), always ending with the residuary relief clause.
   8. After the prayer: "Place:" and "Date:" lines, the advocate/party signature block, and
      the VERIFICATION where the document requires one.
B. NOTICES AND LETTERS (legal notice, cheque-bounce notice, email): letterhead block,
   dispatch mode line ("BY SPEED POST / REGISTERED A.D."), date, addressee block, subject
   line in capitals, numbered body paragraphs, demand + statutory/professional time limit,
   reservation of rights, signature block. NEVER name any court in the body of a notice.
C. DEEDS AND AGREEMENTS (contract, rent agreement, sale deed, vakalatnama): title in
   capitals, execution date/place recital, parties with "hereinafter referred to as ...",
   recitals ("AND WHEREAS ..."), numbered operative clauses, testimonium ("IN WITNESS
   WHEREOF ..."), signature blocks for each party with two attesting witnesses.
D. UNIVERSAL CONVENTIONS:
   - Dates: "05.03.2026", or "the 5th day of March, 2026" in recitals. Never "5/3/26".
   - Money: figures AND words on first mention, Indian numbering — "Rs. 5,00,000/-
     (Rupees Five Lakhs only)".
   - Parties after first mention: "the Petitioner", "the Respondent No. 2" — never bare names.
   - Annexures: mark as "ANNEXURE P-1 / A-1 / R-1" series and reference each in the body
     where the underlying document is first mentioned.
   - One blank line between paragraphs; ALL-CAPS headings on their own line; no markdown,
     no bullet symbols — use numbered or lettered points only.
═══════ END LAYOUT RULES ═══════
`
...
async function chatCompleteRaw(messages, maxTokens = 4000, temperature = 0.25, opts = {}) {
  ...
  for (const model of modelList) {
    ...
    const safeMaxTokens = Math.min(maxTokens, tpmLimit - promptTokens - TPM_SAFETY_MARGIN)
    if (safeMaxTokens < 150) {
    ...
    try {
      ...
      // Request too large for this model's TPM allowance — try the next
      // (usually smaller-context) model rather than failing the whole draft.
      if (msg.includes('rate_limit_exceeded') || msg.includes('Request too large')) {
        console.warn(`[Groq] ${model} rejected request as too large for its TPM limit, trying next...`)
        continue
      }
...
async function generateCourtDocument(messages, maxTokens, temperature, opts = {}) {
  const first = await chatCompleteRaw(messages, maxTokens, temperature, opts)
  ...
  // Drop any trailing meta-commentary paragraph before the final checks.
  text = scrubTrailingMeta(text)
  ...
  // placeholder closing for this document family.
  if (opts.documentType !== 'LEGAL_EMAIL' && !looksStructurallyComplete(text)) {
...
1. TRANSFORM LANGUAGE — PRESERVE FACTS
   - FACTUAL PARTICULARS (names, addresses, dates, amounts, case numbers, FIR numbers, cheque numbers, registration numbers, phone numbers) MUST be reproduced EXACTLY as the user has supplied them. Do not alter proper nouns, numbers, or dates.
   - NARRATIVE / DESCRIPTION: The user will often describe their situation in plain, informal, or colloquial language. Your PRIMARY DUTY is to TRANSFORM that description into precise, elevated legal English as written by a Senior Advocate. Never copy the user's informal phrasing verbatim into the document body. Convert every plain-language sentence into the formal vocabulary, constructions, and gravitas of Indian legal draftsmanship.
   - SUBSTANCE is sacred; PHRASING is yours to elevate. Preserve every fact and allegation the user has stated — do not add new facts or remove stated facts — but re-express everything in impeccable legal language.
   - AUTO-SUPPLY APPLICABLE LAW: Automatically identify and incorporate all relevant statutory provisions, sections, Acts, and legal principles based on the nature of the document and the facts supplied. The user should NOT need to know the applicable sections — that is your expert role.
...
3. PLACEHOLDERS FOR MISSING INPUT
   ...
   - Today's date can be left as "[DATE]" unless the user supplied it. This applies with FULL FORCE to the verification, signature, execution and attestation blocks: NEVER write a specific calendar date there unless the user supplied the signing/verification date — write "[DATE]" / "this [DAY] day of [MONTH], [YEAR]".
   - NEVER reuse a name from the case details in a different role. The surety's, witness's or opposite party's name must never appear as the advocate's or deponent's name. If the advocate's name is not supplied, it is always "[NAME OF ADVOCATE — TO BE FILLED]".
...
5. NO PADDING, NO BLANK SPACES FOR PROVIDED INFO
   - Do not add invented facts, parties, or events. Brevity faithful to the user's input is preferable to elaboration that strays from it.
   - Do NOT leave blank placeholders for information the user has already provided. If the user has filled in a field, use that information — do not re-insert a blank line or [TO BE FILLED] in its place.
...
6. STRUCTURE + LEGAL LANGUAGE = YOUR CONTRIBUTION
   - Your job is to (a) cast the user's facts into the correct document structure with proper headings and numbering, AND (b) express every sentence in precise, elevated legal English — this transformation IS the value of this platform.
   - If the user's input is sparse on facts, leave bracketed placeholders for unknown specifics. But always apply the full standard legal structure, correct statutory provisions, and professional legal language regardless of how informally the user wrote.
   - STANDARD LEGAL DEFAULTS: Apply legally mandated or professionally standard defaults automatically without requiring the user to specify them. Examples: the 15-day statutory period under Section 138(c) of the Negotiable Instruments Act 1881 for cheque-bounce notices; standard consequence language for non-compliance; standard verification clauses. Do not leave these blank.
...
8. PLAIN TEXT ONLY — NO MARKDOWN FORMATTING
   ...
   - Do NOT use the ampersand symbol '&' in document prose — write 'and' in full. (The symbol may appear only inside quoted statute names that already contain it, e.g., "Prevention of Food Adulteration Act".)
   - Do NOT use the percent symbol '%' — write 'per cent' or spell out the fraction in words (e.g., "one per cent", "20 per cent per annum").
   - Do NOT use '@' except inside email addresses. Do NOT use tilde '~', mathematical operators, hash '#', or other symbols in document prose.
   - Do NOT use em-dashes '—' or en-dashes '–' as decorative separators between clauses; use a colon ':', a semicolon ';', or a new paragraph instead. They may appear only inside quoted case citation strings.
...
3. LATIN MAXIMS — use sparingly and only where a maxim genuinely fits the argument,
   defining each in brackets on first use: e.g. audi alteram partem [hear the other side],
   prima facie [at first sight], locus standi [standing to sue], res judicata [already
   adjudicated], ultra vires [beyond lawful power], suo motu [on its own motion],
   inter alia [among other things], mutatis mutandis [with necessary changes].

4. PROFESSIONAL VOCABULARY — draft every sentence in the elevated register of the
   DRAFTING LEXICON supplied below. Never use conversational verbs where the formal
   equivalent exists (said→averred, told→communicated, asked→called upon/prayed,
   broke→contravened, stopped→restrained, unfair→arbitrary and capricious,
   court→this Hon'ble Court, document→instrument, pay→remit/discharge the liability).
...
4. INDEX OF DOCUMENTS — COURT FILINGS ONLY (petitions, applications,
   affidavits, complaints filed in a court/forum; NEVER in a legal
   notice, email, contract, rent agreement or sale deed): after the
   cause title, include an "INDEX OF DOCUMENTS" listing each annexure /
   supporting document with a short description.

5. ANNEXURE LIST — COURT FILINGS ONLY: at the bottom, before signatures,
   include "LIST OF ANNEXURES" with annexure marks (Annexure-A, ...).
   Never add one to a notice, email or deed.

6. VERIFICATION CLAUSE — only for document types that carry one
   (petitions, applications, affidavits, complaints): detailed
   verification (which facts are on personal knowledge, which on
   record, which on belief), separately numbered. Notices, emails and
   deeds do NOT carry a verification clause — deeds close with the
   testimonium and witness attestation instead.

7. PRAYER — COURT FILINGS ONLY: numbered, with main prayer +
   alternative prayers + interim prayer + costs prayer.

8. SECTION HEADINGS: Write ALL top-level section headings in ALL CAPS on
   their own line ("FACTS", "LEGAL ISSUES", "GROUNDS", "PRAYER", etc.) —
   no markdown, no asterisks, no bold markers, no underscores. Use
   numbered sub-headings (1.1, 1.2) for sub-sections.`
...
function jurisdictionOnlyAddendum(court) {
  if (!court) return ''
  return ` JURISDICTION CONTEXT ONLY: the matter arises within the jurisdiction of ${court.replace(/_/g, ' ')}. Use this SOLELY to select the correct state-specific statutes, stamp-duty/registration references and local conventions. THIS DOCUMENT IS NOT A COURT FILING — do NOT name or reference any court in it, do NOT begin it with "IN THE ... COURT" or any cause title, and do NOT add a case-number block.`
}
...
function buildSystemPrompt(documentType, court, language, opts = {}) {
  ...
  const layoutMandate = documentType === 'LEGAL_EMAIL' ? '' : COURT_FORMAT_MANDATE
...
REMINDER: Any name, date, address, amount, FIR number, cheque number, or other particular not present above must be a bracketed placeholder, NOT invented content. Cite every statute by its FULL official name and EXACT section / article number per the CITATION RULES.`
  ...
  try {
    ...
    ], maxTokens, 0.1, { isPro: opts.isPro, documentType, detailsExcerpt: String(details || '').slice(0, 1200) })  // lower temperature → less invention
...
export async function analyzeLegalIssue(issue, court = null) {
  const courtCtx = court
    ? ` Focus on laws and precedents applicable to ${court.replace(/_/g, ' ')}.`
    : ''
  try {
    const text = await chatComplete([
      {
...
• At least one Indian Supreme Court / High Court precedent per argument (party names + year, no fabrication — if unsure, write "[verify citation]")
