...
export async function generateLongCaseBriefStream(rawText, opts = {}, onDelta) {
  ...
  // Free tier (12K TPM on 70B) must fit ALL work in ~50s on Vercel Hobby's
  // 60s function cap: 4 windows × 3.5K chars (~2.8K tokens/call) + 4s
  // spacing + 5s cooldown + 1K-token synthesis stream.
  const windowSize     = isFree ?  3_500 : 10_000
  const windowCount    = isFree ?      4 :       7
  const paceMs         = isFree ?  4_000 :   1_500
  const preSynthDelay  = isFree ?  5_000 :   1_000
  ...
  const HARD_BUDGET_MS = isFree ? 48_000 : 250_000
