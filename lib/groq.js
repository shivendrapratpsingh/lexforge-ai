...
export async function generateShortCaseBriefStream(rawText, opts = {}, onDelta) {
  ...
  // We only enable this for PRO accounts because on Vercel Hobby + free
  // Groq the chunked pipeline keeps blowing the 60s function cap.  Free
  // accounts fall through to the single-call path below, which uses a
  // small HEAD+TAIL slice via buildCaseBriefMessages (auto-scaled to the
  // free-tier TPM budget) and reliably finishes in 15-25s.
  if (opts.isPro && rawText.length > 60_000) {
