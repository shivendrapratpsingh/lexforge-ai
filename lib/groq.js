...
import { activeProviders, clientFor, attemptPlan, primaryModels } from './ai.js'
...
import { actionCatalogueForPrompt, actionExamplesForPrompt, extractAction, ACTION_BLOCK_EXAMPLE } from './assistant-actions.js'
...
// ─── Case law in a draft: retrieved, or nothing ───────────────────
//
// This used to be one fixed block telling the model to name a judgment
// "only if you are certain". That is a self-assessment rule, and the
// same kind of rule had already failed on the chat assistant — the
// model is confident and wrong at the same time, which is precisely
// the failure mode a fabricated citation is.
//
// So the draft is now grounded the way research already was: judgments
// are retrieved from the real index before drafting starts and passed
// in here. The model works from that list and may name nothing else.
// When nothing was retrieved the rule is absolute — no case name at
// all — because a remembered citation is a fabricated one.

// These two blocks are assembled by joining lines rather than by
// embedding newlines in template literals — the prompt text is long,
// heavily punctuated, and an escaped newline that silently becomes a
// real one breaks the file in a way that is hard to see in a diff.
const NL = '\n'

/** The authorities the draft is allowed to rely on, verbatim. */
function authorityBlock({ cases = [], acts = [] } = {}) {
  if (!cases.length && !acts.length) return ''

  const caseLines = cases.length
    ? [
        'RETRIEVED JUDGMENTS — from a real case-law index, for THIS matter.',
        'Refer to them exactly as titled. Quote a citation only as given.',
        '',
        ...cases.map((c, i) => [
          `[${i + 1}] ${c.title}`,
  ...
  const actLines = acts.length
    ? [
        'APPLICABLE STATUTES matched for this matter:',
        ...acts.map(a => {
          const head = `- ${a.fullName || a.shortName}`
          const secs = (a.sections || []).map(x => `    ${x.n}: ${x.desc}`)
          return [head, ...secs].join(NL)
        }),
      ].join(NL)
    : ''
  ...
  return [
    '',
    '═══════ AUTHORITY SUPPLIED FOR THIS DRAFT ═══════',
    [caseLines, actLines].filter(Boolean).join(NL + NL),
    '═══════ END AUTHORITY ═══════',
    '',
  ].join(NL)
}

/** The rule that goes LAST, and says so. */
function caseLawRule(cases = []) {
  const common = `Where any instruction above asks you to "cite precedents", "reference
High Court precedents" or similar, it is satisfied by citing the correct
STATUTORY PROVISIONS. It does NOT authorise you to name judgments of
your own.

A fabricated citation is a professional disaster for the advocate who
files it: it is caught by opposing counsel or the bench, and it is their
name on the document. Zero citations is a good document. One invented
citation ruins it.`

  const body = cases.length
    ? `Judgments HAVE been retrieved for this matter and are listed above under
AUTHORITY SUPPLIED FOR THIS DRAFT.

- You may cite ONLY those judgments, by their exact titles.
- Give a citation only where one is printed above. Never complete or
  reconstruct a citation from memory.
- Do NOT name any other case, however well known — not from memory, not
  as an illustration, not in passing.
- You are not obliged to use all of them. Use the ones that actually bear
  on these facts and ignore the rest.

${common}`
    : `NO judgment has been retrieved for this matter. You therefore have no
case-law index to draw on, and your recollection is not one.

- You may name a judgment ONLY if that exact case is written out, by
  name, somewhere in the instructions above. Those few were checked by a
  person. Nothing else was.
- Do NOT add any other case, however well known, however apt, however
  certain you feel. Your certainty about case law is not evidence — asked
  for authorities on Section 138 this model produced two case names that
  do not exist, confidently and in perfect format.
- Give a citation ONLY where that citation is printed above alongside the
  case name. Never complete, reconstruct or recall one.
- Where an authority is genuinely needed and none is available, write:
      [CITE AUTHORITY — TO BE VERIFIED]
  That placeholder is always correct and is what the advocate expects.

Statutes are different: state sections precisely, per the citation rules.
...
// The provider chain lives in lib/ai.js. This binding serves the call
// sites that pick their own model rather than going through
// chatCompleteRaw's fallback loop — the case-brief pipeline and the
// extraction helpers. It resolves lazily to whichever provider is first
// in AI_PROVIDERS and actually has a key, so those paths follow the same
// switch as everything else without having to know about it.
//
// Lazy, not eager: this module is imported by routes that never call the
// AI at all, and a missing key must not stop them loading.
function primaryClient() {
  const [provider] = activeProviders()
  if (!provider) {
    const e = new Error(
      'No AI provider is configured. Set GEMINI_API_KEY (or GROQ_API_KEY) and redeploy.'
    )
...
const groq = {
  get chat() { return primaryClient().chat },
}
...
// The model lists now come from whichever provider heads the chain, so
// swapping provider is an env change rather than an edit here. The
// literals below are only the floor for the case where nothing is
// configured at all — they keep imports of PRO_MODELS[0] from being
// undefined in modules that read them at load time.
const _primary = primaryModels()
export const PRO_MODELS  = _primary.pro.length  ? _primary.pro  : ['openai/gpt-oss-120b', 'openai/gpt-oss-20b']
export const FREE_MODELS = _primary.free.length ? _primary.free : ['openai/gpt-oss-20b', 'openai/gpt-oss-120b']

// Reasoning models emit thinking tokens before their answer, and that
// thinking is charged against max_tokens — so a request with a small
// budget comes back as an empty string. Measured on gpt-oss: a 20-token
// request produced "" while spending ~560 tokens thinking. Both Groq's
// gpt-oss models and Gemini accept `reasoning_effort`, so this is now a
// property of the provider rather than a hardcoded model list.
...
// The ceiling now travels with the provider (lib/ai.js): Groq's free
// plan is 8,000, Gemini's paid tier is 250,000. That difference is the
// whole reason a Pro draft in Hindi could not be produced at all — the
// prompt left negative headroom under 8,000 and every model was skipped.
...
// Which models are known to be over their limit, and until when.
//
// Each model has its OWN 8,000 tokens-per-minute bucket, so when one is
// exhausted the other still has room. Without this the list is tried in
// a fixed order, so under load every single request burns a round-trip
// being refused by the first model before falling back to the second —
// paying the latency of a failure on every call, all day.
//
// In-process only. Serverless spreads requests across instances, so this
// is a per-instance memory rather than a global one — which is still
// worth having, because an instance handling requests back to back is
// exactly the case that trips the limit.
const modelCooldown = new Map()
const COOLDOWN_MS = 60_000   // the TPM window Groq measures over

function markRateLimited(model) {
  modelCooldown.set(model, Date.now() + COOLDOWN_MS)
}

// Models with headroom first, cooling ones last — last rather than
// removed, because a stale cooldown must never leave nothing to try.
function orderByCooldown(models) {
  const now = Date.now()
  const ready = []
  const cooling = []
  for (const m of models) {
    ;(modelCooldown.get(m) > now ? cooling : ready).push(m)
  }
  return [...ready, ...cooling]
}

async function chatCompleteRaw(messages, maxTokens = 4000, temperature = 0.25, opts = {}) {
  // Every model of the first provider, then every model of the second.
  // A provider without a key never appears, so this is safe to run
  // before any Gemini key exists — the chain simply resolves to Groq.
  const plan = attemptPlan({ isPro: opts.isPro })
  if (!plan.length) {
    const e = new Error(
      'No AI provider is configured. Set GEMINI_API_KEY (or GROQ_API_KEY) and redeploy.'
    )
  ...
  // Cooldown ordering still applies, but within the plan rather than
  // over a bare model list — a model cooling on one provider must not
  // reorder a different provider's models.
  const order = orderByCooldown(plan.map(a => `${a.provider.id}:${a.model}`))
  const byKey = new Map(plan.map(a => [`${a.provider.id}:${a.model}`, a]))
  ...
  for (const key of order) {
    ...
    // probabilistic one.
    const tpmLimit = attempt.tpm || DEFAULT_TPM_LIMIT
    ...
    if (headroom < 150) {
      console.warn(`[ai] ${provider.id}: prompt alone (~${promptTokens} tokens) leaves no room under ${model}'s ${tpmLimit} TPM limit, skipping...`)
      continue
    }
    ...
    const wanted = attempt.reasoning
      ? Math.max(maxTokens, MIN_REASONING_BUDGET)
      : maxTokens
    const safeMaxTokens = Math.min(wanted, headroom)

    try {
      const completion = await clientFor(provider).chat.completions.create({
        messages,
        model,
        temperature,
        ...
        ...(attempt.reasoning ? { reasoning_effort: 'low' } : null),
      })
      ...
      if (status === 429 || status === 413 || /rate.?limit|tokens per minute|\bTPM\b|Request too large|quota/i.test(msg)) {
        sawRateLimit = true
        markRateLimited(key)
        // Recorded as a failed call. Nothing wrote failures before, so a
        // total AI outage left no trace in the database whatsoever —
        // which is precisely how Groq retiring both Llama models went
        // unnoticed for months. A success rate means nothing if only
        // successes are counted.
        trackUsage({ userId: opts.userId ?? null, provider: provider.id, operation: opts.operation || 'generate', ok: false })
        console.warn(`[ai] ${provider.id}:${model} rate-limited / over TPM — trying next...`)
        continue
      }
      // Insufficient prepaid credit for the budget we asked for.
      //
      // OpenRouter reserves the MAXIMUM possible cost of a request
      // against the balance before running it, so a large max_tokens is
      // refused outright when the balance is thin — even though the
      // completion would have cost a fraction of it. The message names
      // the number we could afford:
      //
      //   402 ... You requested up to 8500 tokens, but can only
      //   afford 7701.
      //
      // Falling straight through to the next provider wastes a working
      // one over an arithmetic detail, so retry this same model once at
      // the budget it will accept. A shorter document beats no document,
      // and the user still gets told nothing when it succeeds.
      if (status === 402 || /can only afford|requires more credits/i.test(msg)) {
        const afford = Number(String(msg).match(/afford\s+(\d+)/i)?.[1] || 0)
        const retryBudget = Math.min(safeMaxTokens, Math.floor(afford * 0.95))
        if (afford > 400 && retryBudget < safeMaxTokens && !attempt.__retriedForCredit) {
          attempt.__retriedForCredit = true
          console.warn(`[ai] ${provider.id}:${model} credit-limited — retrying at ${retryBudget} tokens (balance covers ${afford})`)
          try {
            const completion = await clientFor(provider).chat.completions.create({
              messages, model, temperature, max_tokens: retryBudget,
              ...(attempt.reasoning ? { reasoning_effort: 'low' } : null),
            })
            const text = completion.choices?.[0]?.message?.content
            if (text && text.trim().length > 10) {
              ...
              trackUsage({
                userId: opts.userId ?? null, provider: provider.id,
                operation: opts.operation || 'generate',
                tokens: completion.usage?.total_tokens ?? null,
              })
              // Same shape as the normal success path — a different one
              // here would break generateCourtDocument's continuation
              // logic, which reads finishReason to decide whether the
              // document was cut off mid-sentence.
              return { text, finishReason: completion.choices?.[0]?.finish_reason || 'stop' }
            }
          } catch (e2) {
            console.warn(`[ai] ${provider.id}:${model} retry at reduced budget also failed: ${String(e2?.message).slice(0, 90)}`)
          }
        }
        trackUsage({ userId: opts.userId ?? null, provider: provider.id, operation: opts.operation || 'generate', ok: false })
        console.warn(`[ai] ${provider.id}: out of prepaid credit — top up at openrouter.ai/credits`)
        continue
      }
      ...
      if (status === 404 || /model|not found|deprecated|decommissioned/i.test(msg)) {
        trackUsage({ userId: opts.userId ?? null, provider: provider.id, operation: opts.operation || 'generate', ok: false })
        console.warn(`[ai] ${provider.id}: model ${model} unavailable (status ${status}) — trying next... ${String(msg).slice(0, 120)}`)
        continue
      }
      // Anything else — try next model but record it.
      lastError = err
      trackUsage({ userId: opts.userId ?? null, provider: provider.id, operation: opts.operation || 'generate', ok: false })
      console.warn(`[ai] ${provider.id}:${model} error: ${msg}`)
      continue
    }
  }
...
// Closing markers, in every language the app drafts in.
//
// This was English-only. A Telugu bail application ends with a native
// verification over a native word for deponent and matched none of the
// English terms below — it passed only because the bracketed
// placeholders the model leaves behind ([NAME OF ADVOCATE — TO BE
// FILLED]) are still written in English. That is luck, not detection:
// localise those placeholders and every non-English filing would read
// as truncated and be sent back for a continuation it does not need.
// The native terms are listed so the check stands on the document
// rather than on an artefact of it.
const CLOSING_MARKERS = new RegExp([
  String.raw`\b(VERIFICATION|VERIFIED AT|DEPONENT|ADVOCATE|SIGNATURE|IN WITNESS WHEREOF|EXECUTION|WITNESSES?:|Place\s*:|Date\s*:)`,
  'सत्यापन|अभिसाक्षी|डिपोनेंट|अधिवक्ता|हस्ताक्षर|सत्यापित',
  'சரிபார்ப்பு|வழக்கறிஞர்|கையொப்பம்|உறுதிமொழி',
  'ధృవీకరణ|ధృవీకరిస్తున్నాను|న్యాయవాది|డిపోనెంట్|సంతకం',
  'ಪರಿಶೀಲನೆ|ವಕೀಲ|ಸಹಿ|ದೃಢೀಕರಣ',
  'تصدیق|وکیل|دستخط',
].join('|'), 'i')
...
WHAT AN AFFIDAVIT MUST NOT CONTAIN. An affidavit is sworn evidence of FACT and nothing else. It carries NO PRAYER CLAUSE and asks the court for nothing — the relief is sought in the application or petition the affidavit supports, and a deponent who prays for relief on oath has confused the two documents. Do not write "PRAYER", "it is most respectfully prayed", or any request for an order, stay, direction or costs.

NOTHING FOLLOWS THE ATTESTATION. The verification, the DEPONENT signature line and the Oath Commissioner block are the end of the document. Do not place annexures, a prayer, grounds or submissions after them. A page appearing beneath a sworn signature reads as material inserted after the oath was administered, which is exactly what the form exists to prevent.`,
...
function buildSystemPrompt(documentType, court, language, opts = {}) {
  ...
  return (
    ...
    tierAddendum +
    proRegister +
    authorityBlock({ cases: opts.cases, acts: opts.acts }) +
    caseLawRule(opts.cases)
  )
}

// The prompt builder is internal, but the citation rule inside it is a
// promise made to a college and to every advocate who files the output.
// scripts/verify-grounding.mjs reads the assembled prompt through this.
export const buildDraftPromptForTest = (documentType, court, language, opts) =>
  buildSystemPrompt(documentType, court, language, opts)
...
REMINDER: Any name, date, address, amount, FIR number, cheque number, or other particular not present above must be a bracketed placeholder, NOT invented content. Cite every statute by its FULL official name and EXACT section / article number per the CITATION RULES.${
  ...
  try {
    ...
    // An empty completion is a FAILURE, not a document.
    //
    // This line used to read `return text || fallback(...)`, which meant
    // that when the model returned "" the user silently received a
    // template of their own form fields, formatted, dated, and signed
    // "Generated by LexForge AI" at the bottom. It looked like a
    // deliverable. A user reported it as "it only shows what I typed" —
    // and they were right, because that is exactly what it was.
    //
    // Under an 8,000-token ceiling this was not rare: a Pro draft in
    // Hindi left negative headroom, so it happened on every single
    // attempt. Failing loudly is the only honest option — the caller
    // still has the user's input and can offer a retry.
    if (!text) {
      const e = new Error(
        'The AI returned an empty response. Nothing was saved — please try generating again.'
      )
      e.code = 'AI_EMPTY'
      throw e
    }
    ...
    if (err?.code === 'AI_REFUSAL' || err?.code === 'GROQ_RATE_LIMIT' ||
        err?.code === 'GROQ_AUTH'  || err?.code === 'AI_EMPTY' ||
        err?.code === 'NO_AI_PROVIDER') throw err
...
// The assistant is a CHAT surface with no retrieval behind it, which
// makes it the one place where "cite a case only if you are certain" is
// not a workable rule. A model's certainty is not calibrated: asked for
// authorities on Section 138 it produced "M/s. R. K. Industries v. M/s.
// R. K. Traders", then "Mohan v. State of Karnataka" — confident, well
// formatted, and almost certainly not real.
//
// So the prohibition here is absolute rather than conditional. Elsewhere
// in the app the model may discuss judgments because judgments were
// RETRIEVED and handed to it. Nothing is retrieved here, so nothing may
// be named. The user is sent to Case Law search, which queries the real
// index and returns real citations.
const ASSISTANT_CASE_RULE = `
═══════ CASE NAMES — ABSOLUTE, OVERRIDES EVERY INSTRUCTION ABOVE ═══════
You have NO case database in this conversation. No judgment has been
retrieved for you. Therefore:

NEVER write a case name. Not "X v. Y", not "X vs State", not a party
name, not a reporter citation, not a year of decision — no matter how
confident you feel. Your confidence about case law is not evidence, and
in this chat it has been wrong every time it has been checked.

Statutes are different: you may and should state sections precisely.

Where an authority is genuinely needed, write exactly:
    [AUTHORITY NEEDED — <the proposition, in a few words>]
then tell the user: "Search that in Case Law and you will get the real
judgment with its real citation."

A lawyer who files a case name you invented loses the matter and faces
the bench personally. Naming nothing is always the safe answer; naming
something plausible never is.
═══════════════════════════════════════════════════════════════════════`
...
// ─── Routing: turn an answer into a destination ────────────────────
//
// Answering "which Act applies?" well and then leaving the user to find
// New Document, pick the right tile out of twenty, and retype every fact
// they just explained is where most people give up. So the assistant
// also names a destination and hands over the particulars it heard.
//
// The catalogue is generated from lib/assistant-actions.js, so the menu
// the model sees can never drift from the destinations that exist.
//
// It PROPOSES only. Nothing navigates on its own.

function assistantRoutingRule() {
  return [
    '',
    '─── HANDING THE WORK OVER (required) ───',
    'Your answer is not the end of the exchange. The user still has to',
    'find the right page in this app and retype every fact they just',
    'told you. So END EVERY REPLY that responds to an actual request with one machine-readable block naming where to send them:',
    '',
    ...
    '',
    'RULES:',
    '1. Put the block on its own line at the very END, after your prose.',
    '   Never mention it, never explain it, never describe a button. The',
    '   interface turns it into one; the user must not read about it.',
    '2. Fill args ONLY with particulars the user actually gave you, in',
    '   their own words. Do NOT infer a name, date, amount, section,',
    '   court or address that was not stated. Leave a field out rather',
    '   than guessing it. A prefilled form that quietly invents a date is',
    '   worse than an empty one, because nobody re-reads a field that is',
    '   already filled. Args may be empty: a block with only the',
    '   destination is still useful and still correct.',
    '3. Use ONLY the exact values listed above. A near-miss is rejected',
    '   and silently costs the user the handover, so if you cannot name',
    '   the exact value, name a different destination you can.',
    '4. If two destinations fit, pick the one that produces the thing the',
    '   user needs NEXT — the document over the statute, the analysis of the order they were sent over a search about it.',
    '5. Exactly one block. Omit it ONLY for pure small talk, thanks, or a',
    '   question about this app itself.',
  ].join('\n')
}
...
export async function caseAssistant(messages, draftContext = null, opts = {}) {
  ...
  if (safeMessages.length === 0) {
    return { reply: 'Hi! I am your Case Assistant. Tell me what has happened and I will tell you which law applies — and offer to open the right document with your details already filled in.', action: null }
  }
  ...
  try {
    const text = await chatComplete([
      // The case-law rule goes LAST. It says it overrides everything above
      // it, and a trailing instruction is the one a model actually
      // follows — which is the whole reason it sits here rather than
      // with the other mandates. Drafting, analysis and the moot builder
      // were fixed this way; the assistant was missed, and was still
      // inventing case names as a result.
      { role: 'system', content: ASSISTANT_SYSTEM + CITATION_MANDATE + contextBlock + (lawsBlock ? `\n\n${lawsBlock}` : '') + ASSISTANT_CASE_RULE + assistantRoutingRule() },
      ...safeMessages,
    ], 1200, 0.3, { isPro: true })  // assistant always uses Pro models
    if (!text) return { reply: 'Sorry, I could not generate a response. Please try rephrasing.', action: null }
    return extractAction(text)
  } catch (err) {
    console.error('caseAssistant error:', err)
    if (err?.code === 'AI_REFUSAL') {
      return { reply: 'I cannot help with that specific request. Please rephrase or check your case details for inappropriate language.', action: null }
    }
    return { reply: 'The assistant is temporarily unavailable. Please try again in a moment.', action: null }
  }
}
  ...
  for (let i = 0; i < windows.length; i++) {
    ...
    while (attempt < 2) {
      try {
        ...
        if (status === 429 || status === 413 || /rate_limit|tokens per minute|TPM|Request too large|quota/i.test(msg)) {
          attempt += 1
...
• Never invent citations. If unsure, write "[verify in latest edition of <book>]" rather than fabricating.`
  ...
  try {
    // 1,200 was too small and the failure was invisible. The prompt asks
    // for ~400 words, but 400 words of legal prose carrying markdown
    // headings and full case citations - "***Arjun Panditrao Khotkar v.
    // Kailash Kushanrao Gorantyal (2020) 7 SCC 1:***" is thirty tokens on
    // its own - runs past 1,200. A measured answer stopped dead at
    // "Anvar P.V. v. P.K. Basheer (2014", mid-citation.
    //
    // chatCompleteRaw rather than chatComplete because chatComplete
    // discards finishReason, which is the only way to know this happened.
    const result = await chatCompleteRaw([
      { role: 'system', content: system },
      { role: 'user',   content: user   },
    ], opts.isPro === false ? 1400 : 2400, 0.25, { isPro: opts.isPro })

    const text = result?.text
    if (!text) return 'No answer generated. Please try again.'
    if (result.finishReason !== 'length') return text

    // Cut off at the ceiling. A half-written citation is worse than a
    // shorter answer - a student copies "Anvar P.V. v. P.K. Basheer
    // (2014" into an exam and it is wrong in a way they cannot see. So
    // roll back to the last line that finished, and say what happened.
    return trimToLastCompleteLine(text) +
      NL + NL + '_(Answer shortened to the last complete point. Ask a narrower question for the rest.)_'
  } catch (err) {
    console.error('answerLegalQuestion error:', err)
    return 'Unable to answer right now. Please try again in a moment.'
  }
}

/**
 * Drop a trailing fragment left by hitting the token ceiling.
 *
 * Prefers the last completed line, falling back to the last completed
 * sentence, and returns the text unchanged if neither can be found -
 * losing everything to tidiness would be the worse trade.
 */
function trimToLastCompleteLine(raw) {
  const text = String(raw || '').trimEnd()
  const lines = text.split(NL)

  // A line that ends in sentence punctuation, or is a heading, finished.
  const finished = (l) => /[.!?:]["')\]]?$/.test(l.trim()) || /^#{1,6}\s/.test(l.trim()) || l.trim() === ''
  let last = lines.length - 1
  while (last >= 0 && !finished(lines[last])) last--
  if (last >= 0) {
    const kept = lines.slice(0, last + 1).join(NL).trimEnd()
    if (kept.length > text.length * 0.4) return kept
  }

  const stop = Math.max(text.lastIndexOf('. '), text.lastIndexOf('.' + NL))
  return stop > text.length * 0.4 ? text.slice(0, stop + 1) : text
}
...
// Renders whatever a caller passed as "details" into something a person
// can read: a string stays a string, an object becomes labelled lines.
function formatDetails(details) {
  if (!details) return '[No details supplied]'
  if (typeof details === 'string') return details.trim() || '[No details supplied]'

  if (typeof details === 'object') {
    const lines = Object.entries(details)
      .filter(([, v]) => v != null && String(v).trim())
      .map(([k, v]) => {
        // camelCase / snake_case field names into something readable.
        const label = k.replace(/[_-]+/g, ' ').replace(/([a-z])([A-Z])/g, '$1 $2')
        const pretty = label.charAt(0).toUpperCase() + label.slice(1)
        return `${pretty}: ${typeof v === 'object' ? JSON.stringify(v) : String(v).trim()}`
      })
...
function fallback(type, details) {
  // The date of execution on a document someone may file. Rendered in
  // IST rather than the server's UTC, which was a day out every night
  // between midnight and half past five.
  const date = formatDate()
  const docName = String(type || 'DOCUMENT').replace(/_/g, ' ').toUpperCase()
  return [
    '*** THIS IS NOT A FINISHED DOCUMENT — DO NOT FILE OR SEND IT ***',
    '',
    'The AI could not be reached, so nothing was drafted. What follows is',
    'only the information you typed, kept so that none of it is lost.',
    'Generate again to get the actual document.',
    '',
    docName + '  (UNDRAFTED)',
    '',
    'Date: ' + date,
    ...
    '──────────────────────────────────────────────────',
    '',
    // `details` arrives as an object from some callers and a string from
    // others. String(anObject) is "[object Object]" — which is what a
    // user got, printed into the one part of this document that exists
    // to preserve their facts verbatim. The whole point of the fallback
    // is that the input survives when the AI does not.
    formatDetails(details),
    '',
    '──────────────────────────────────────────────────',
    '',
    '*** END OF UNDRAFTED INPUT — THIS IS NOT A LEGAL DOCUMENT ***',
  ].join('\n')
}

// ─── Case comment on an uploaded judgment ─────────────────────────
//
//  A student uploads a judgment nobody has briefed for them and gets
//  back the same structure as the landmark cases that ship with the
//  app, plus the parts a case COMMENT needs and a brief does not: how
//  the court reasoned, what was obiter, who dissented, and what the
//  judgment can fairly be criticised for.
//
//  This is NOT generateShortCaseBrief. That reads a live case FILE for
//  a lawyer — parties, chronology, strategy. This reads one REPORTED
//  JUDGMENT for a student who has to write about it and then defend
//  what they wrote.
//
//  THE FIDELITY RULE IS THE WHOLE FEATURE. Everything must come out of
//  the uploaded text. A student cannot tell an invented ratio from a
//  real one — that is precisely why they uploaded the judgment — and a
//  wrong ratio repeated in a viva is worse than an admitted gap. So the
//  prompt requires the model to record an absence rather than supply it
//  from memory of a case it may well recognise.

const CASE_COMMENT_FIELDS = [
  'name', 'year', 'citation', 'court', 'area',
  'facts', 'issue', 'holding', 'ratio', 'importance',
  'reasoning', 'obiter', 'dissent', 'authorities', 'critique', 'examFocus',
]

/**
 * Sample a long judgment down to something that fits the budget.
 *
 * Mirrors the approach in buildCaseBriefMessages, but for ONE document
 * rather than a folder. A judgment puts its facts and issues at the
 * front and its holding and operative order at the back, with pages of
 * counsel's arguments in between — so head and tail carry most of what
 * a comment needs. Free accounts get evenly spaced windows instead,
 * because their per-minute token ceiling will not take head plus tail.
 */
function sampleJudgment(raw, { isPro } = {}) {
  const text = String(raw || '')
  if (isPro === false) {
    const COUNT = 14, CHARS = 700
    if (text.length <= COUNT * CHARS + 500) return text
    const denom = COUNT - 1
    const pieces = []
    for (let i = 0; i < COUNT; i++) {
      const start = Math.floor((i * (text.length - CHARS)) / denom)
      pieces.push('== part ' + (i + 1) + ' of ' + COUNT + ' ==' + NL + text.slice(start, start + CHARS))
    }
    return pieces.join(NL + NL)
  }
  const HEAD = 34000, TAIL = 16000
  if (text.length <= HEAD + TAIL + 200) return text
  return text.slice(0, HEAD) +
    NL + NL + '[... middle of the judgment omitted for length ...]' + NL + NL +
    text.slice(-TAIL)
}

const CASE_COMMENT_SYSTEM = `You are a law professor writing a CASE COMMENT for an Indian law student who has to understand this judgment well enough to defend it in a viva.

THE ABSOLUTE RULE
Everything you write must come from the judgment text supplied. You may recognise this case. It does not matter. If the supplied text does not contain something, write exactly "Not stated in the judgment as supplied." Do NOT fill a gap from memory, and do NOT name a case, section or judge that does not appear in the text. A student cannot tell an invented ratio from a real one — that is why they uploaded the judgment — and a wrong ratio repeated in a viva is worse than an admitted gap.

WRITE FOR SOMEONE LEARNING, NOT SOMEONE CHECKING
Do not compress into note form. Explain. When you use a term of art — ratio decidendi, obiter dicta, per incuriam, locus standi — say in the same breath what it means here. Where the reasoning turns on a distinction, spell the distinction out. Assume an intelligent second-year who has not read this judgment.

DEPTH EXPECTED
"facts" and "reasoning" should each run to several full paragraphs. "ratio", "obiter" and "critique" are what separate a case comment from a summary — give them real weight. Thin fields are a failure, not brevity.

Return ONLY a JSON object. No prose around it, no markdown fences.

{
  "name": "full case name as it appears in the judgment",
  "year": 1973,
  "citation": "the reported citation IF the text contains one, else an empty string",
  "court": "the court, and the bench strength if stated",
  "area": "one of: Constitutional, Criminal, Civil, Contract, Property, Family, Company, Tax, Labour, Environment, Consumer, Administrative, Evidence, Other",
  "facts": "What happened, told as a story a student can follow. Who the parties were, what dispute arose, how it travelled through the courts to reach this bench. Several paragraphs.",
  "issue": "The precise legal questions the court set itself. Number them if there is more than one, and frame each as a question.",
  "holding": "What the court actually decided on each issue, and the split if the bench divided.",
  "ratio": "The ratio decidendi — the principle that binds future courts. State it, then explain WHY that is the ratio and not merely something the court happened to say. This is the single most important field.",
  "reasoning": "How the court travelled from the issue to the holding, step by step: the provisions it read, the earlier decisions it accepted or distinguished, the arguments it rejected and on what ground. Several paragraphs.",
  "obiter": "Observations not necessary to the decision, which therefore do not bind. Say why each is obiter rather than ratio. If there is none worth noting, say so.",
  "dissent": "The minority view if the judgment carries one — who dissented and on what reasoning. If the bench was unanimous, say so.",
  "authorities": "Cases, statutes and constitutional provisions the judgment itself relies on or distinguishes. ONLY those appearing in the supplied text.",
  "critique": "What a thoughtful commentator would say about this judgment: what it settles, what it leaves open, where the reasoning is vulnerable, what problems it creates for later courts. Fair, not hostile.",
  "importance": "Why this judgment matters and what changed because of it.",
  "examFocus": "How this case is asked in examinations, and the two or three points a student must be able to state without hesitation."
}`

/**
 * Produce a structured case comment from a judgment's text.
 * Returns an object keyed by CASE_COMMENT_FIELDS, or throws.
 */
export async function generateCaseComment(rawText, opts = {}) {
  const source = String(rawText || '').trim()
  if (source.length < 400) {
    throw Object.assign(
      new Error('That is too short to be a full judgment — upload the complete text.'),
      { code: 'SOURCE_TOO_SHORT' },
    )
  }

  const trimmed = sampleJudgment(source, opts)
  const text = await chatComplete([
    { role: 'system', content: CASE_COMMENT_SYSTEM },
    { role: 'user', content: 'Write the case comment on this judgment.' + NL + NL + trimmed },
  ], opts.isPro === false ? 2600 : 5000, 0.2,
     { isPro: opts.isPro, operation: 'case-comment', userId: opts.userId })

  if (!text) throw Object.assign(new Error('The model returned nothing.'), { code: 'AI_EMPTY' })

  let parsed = null
  try {
    parsed = JSON.parse(text)
  } catch {
    // Models wrap JSON in prose or a fence often enough that two other
    // places in this file already recover the same way.
    const first = text.indexOf('{')
    const last = text.lastIndexOf('}')
    if (first !== -1 && last > first) {
      try { parsed = JSON.parse(text.slice(first, last + 1)) } catch { /* give up below */ }
    }
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw Object.assign(
      new Error('The case comment came back in a form we could not read. Try again.'),
      { code: 'BAD_SHAPE' },
    )
  }

  // Keep only declared fields, and coerce to strings, so the UI never
  // has to render something it did not expect.
  const out = {}
  for (const key of CASE_COMMENT_FIELDS) {
    const v = parsed[key]
    if (v === undefined || v === null || v === '') continue
    out[key] = key === 'year' ? (Number(v) || null) : String(v).trim()
  }
  if (!out.name) out.name = 'Uploaded judgment'
  return out
}

export { CASE_COMMENT_FIELDS }
