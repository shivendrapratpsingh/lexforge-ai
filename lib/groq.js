...
async function generateCourtDocument(messages, maxTokens, temperature, opts = {}) {
  ...
  if (!looksStructurallyComplete(text)) {
    const repair = await chatCompleteRaw([
      {
        role: 'system',
        content: 'An Indian court document is missing its closing formalities. Given the excerpt below, write ONLY the missing closing section(s) needed to complete it — the verification clause and the advocate/deponent signature block (or execution/witness block, for instruments that use one) appropriate to this kind of document — so it can be appended directly after the text. Do not repeat any earlier content.',
      },
      { role: 'user', content: `END OF DOCUMENT SO FAR:\n${text.slice(-1500)}` },
    ], 500, temperature, opts).catch(() => null)
