...
import { actionCatalogueForPrompt, extractAction, ACTION_BLOCK_EXAMPLE } from './assistant-actions.js'
...
function assistantRoutingRule() {
  return [
    ...
    "After your answer, if the user's message points clearly at ONE thing",
    'this app can do for them, append a single machine-readable block:',
    '',
    ...
    '',
    'RULES, and they matter more than the suggestion does:',
    '1. Put the block on its own line, AFTER your prose answer. Never',
    '   mention it, never explain it, never describe the button. The',
    '   interface renders it; the user must not read about it.',
    '2. Fill args ONLY with particulars the user actually gave you, in',
    '   their own words. Do NOT infer a name, date, amount, section,',
    '   court or address that was not stated. A prefilled form that',
    '   quietly invents a date is worse than an empty one, because',
    '   nobody re-reads a field that is already filled.',
    '3. Use ONLY the exact values listed above. A near-miss is not',
    '   accepted and silently costs the user the button entirely, so if',
    '   you cannot name the exact value, omit the whole block.',
    '4. Emit NOTHING if you are unsure which destination is right, if the',
    '   user is only chatting, or if they have not given you enough to',
    '   act on. One correct suggestion is worth more than five plausible',
    '   ones — a wrong button sends somebody where they did not ask to go.',
    '5. At most one block per reply.',
  ].join('\n')
}
