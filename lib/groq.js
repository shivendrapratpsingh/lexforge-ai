...
async function generateCourtDocument(messages, maxTokens, temperature, opts = {}) {
  let first = await chatCompleteRaw(messages, maxTokens, temperature, opts)
  // Bounded retry: if the FIRST attempt fully failed (all models
  // rate-limited / momentarily unavailable), wait for the per-minute TPM
  // bucket to refill a little and try ONCE more before giving up to the
  // raw-template fallback. On a free Groq key a transient 429 is common
  // when the user generated another document seconds earlier; this single
  // retry converts most of those into a real, court-formatted draft. The
  // ~9s wait keeps total latency inside the 60s serverless ceiling.
  if (!first) {
    await sleep(9000)
