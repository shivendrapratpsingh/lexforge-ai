...
function buildCaseBriefMessages(rawText, opts = {}) {
  ...
  const shrink = typeof opts.shrink === 'number' && opts.shrink > 0 && opts.shrink < 1
    ? opts.shrink
    : 1
  const HEAD = Math.max(800,  Math.floor((isFree ? 4_000  : 30_000) * shrink))
  const TAIL = Math.max(400,  Math.floor((isFree ? 2_000  : 15_000) * shrink))
...
export async function generateLongCaseBriefStream(rawText, opts = {}, onDelta) {
  ...
  // Free tier (12K TPM on 70B) must fit total work in ~55s:
  //   5 windows × 3K chars (~2.8K tokens/call) + 5s spacing + 1K synth.
  const windowSize     = isFree ?  3_000 : 10_000
  const windowCount    = isFree ?      5 :       7
  const paceMs         = isFree ?  5_000 :   1_500
  const preSynthDelay  = isFree ?  7_000 :   1_000
  const synthMaxTokens = isFree ?  1_000 :   4_000
    ...
    while (attempt < 4) {
      try {
        ...
        if (status === 429 || status === 413 || /rate_limit|tokens per minute|TPM|Request too large/i.test(msg)) {
          attempt += 1
          const waitMs = 15_000 + attempt * 5_000
          emitMeta({ kind: 'wait', seconds: Math.round(waitMs / 1000), reason: 'rate-limit cooldown' })
          await sleep(waitMs)
          continue
        }
        // unknown failure on this window — note and continue with what we have
        digest = `(extraction failed for this section: ${msg.slice(0, 120)})`
        break
      }
    }
...
export async function generateShortCaseBriefStream(rawText, opts = {}, onDelta) {
  ...
  // Long documents → chunked map-reduce so the entire file is analysed
  // without blowing past per-minute token limits.
  const LONG_DOC_THRESHOLD = opts.isPro === false ? 25_000 : 60_000
  if (rawText.length > LONG_DOC_THRESHOLD) {
    return generateLongCaseBriefStream(rawText, opts, onDelta)
  }
  const isFree   = opts.isPro === false
  const baseMax  = isFree ? 1200 : 4000          // tighter for free TPM budgets
  const modelList = opts.isPro ? PRO_MODELS : (isFree ? FREE_MODELS : MODELS)

  // We retry on rate-limits by shrinking the input slice (HEAD/TAIL) and the
  // output budget. Each pass is roughly half the size of the previous.
  const attempts = [
    { shrink: 1.0,  maxTokens: baseMax  },
    { shrink: 0.5,  maxTokens: Math.max(800, Math.floor(baseMax * 0.7)) },
    { shrink: 0.25, maxTokens: 700 },
  ]

  let lastErr = null
  for (const { shrink, maxTokens } of attempts) {
    const { system, user } = buildCaseBriefMessages(rawText, { ...opts, shrink })
  ...
  if (lastErr) {
    ...
    if (status === 429 || status === 413 || msg.includes('tokens per minute') || msg.includes('TPM') || msg.includes('rate_limit')) {
      const e = new Error(
        'The case file is too large for your current AI quota. The brief was shrunk twice but still exceeded the per-minute token limit on your Groq free-tier plan. Wait ~60 seconds and try again, or upgrade your Groq account to a paid tier for higher limits.'
      )
      e.code = 'GROQ_RATE_LIMIT'
      throw e
    }
