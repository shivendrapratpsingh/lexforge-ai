...
You must draft this document to the highest standard of Indian legal writing, matching the style and precision of Senior Advocates and Solicitor General arguments before the Hon'ble Supreme Court of India. Apply the following rules without exception:
...
function buildSystemPrompt(documentType, court, language) {
  const base = BASE_PROMPTS[documentType] || BASE_PROMPTS.LEGAL_NOTICE
  return base + SUPREME_COURT_STYLE + courtAddendum(court) + languageAddendum(language)
}
...
export async function generateLegalDocument(documentType, details, court = null, language = 'english', opts = {}) {
  ...
  const jurisdictionNote = isTNcourt
    ? 'Tamil Nadu / Madras High Court jurisdiction — cite Tamil Nadu statutes and Madras HC / Supreme Court of India precedents.'
    : 'Uttar Pradesh / Allahabad High Court jurisdiction — cite UP statutes and Allahabad HC / Supreme Court of India precedents.'

  const styleNote = `
QUALITY REQUIREMENT: This document must read as if drafted by a Senior Advocate of the Supreme Court of India with impeccable credentials. Every paragraph must demonstrate deep mastery of Indian law. Use elevated legal vocabulary, embed at least 3–5 Latin maxims where contextually appropriate (defined in brackets), cite real and authoritative Supreme Court / High Court precedents with proper neutral citations, and ensure the prayer is majestic and exhaustive. The final document should command immediate respect from any judicial officer who reads it.`
  ...
  try {
    const text = await chatComplete([
      ...
      {
        role: 'user',
        content: `Draft a complete, court-ready ${documentType.replace(/_/g, ' ')}${courtLabel}${langNote} using the following case details:\n\n${details}\n\nJurisdiction: ${jurisdictionNote}\n\n${styleNote}`,
      },
    ], 4000, 0.2, { isPro: opts.isPro })
...
export async function generateMeritsDemerits(documentType, content, templateData = {}, court = null) {
  const isTNcourt = court && (court.startsWith('MADRAS_HC') || court.startsWith('MADURAI_BENCH') || court.startsWith('TN_'))
