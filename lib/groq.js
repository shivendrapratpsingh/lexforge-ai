...
export async function generateLegalDocument(documentType, details, court = null, language = 'english', opts = {}) {
  ...
  const courtLabel = court ? court.replace(/_/g, ' ') : 'the specified court'
  const jurisdictionNote = isTNcourt
    ...
    : court
      ? `${courtLabel} jurisdiction — cite the applicable state statutes and the High Court / Supreme Court of India precedents that govern this court (real ones only).`
