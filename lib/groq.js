...
async function chatCompleteRaw(messages, maxTokens = 4000, temperature = 0.25, opts = {}) {
  const modelList = orderByCooldown(
    opts.isPro ? PRO_MODELS : (opts.isPro === false ? FREE_MODELS : MODELS)
  )
  ...
  for (const model of modelList) {
    ...
    try {
      ...
      if (status === 429 || status === 413 || /rate.?limit|tokens per minute|\bTPM\b|Request too large|quota/i.test(msg)) {
        ...
        trackUsage({ userId: opts.userId ?? null, provider: 'groq', operation: opts.operation || 'generate', ok: false })
...
function fallback(type, details) {
  ...
  return [
    ...
    '──────────────────────────────────────────────────',
    '',
    '[NOTE: the AI was unavailable when this document was produced, so',
    ' your case particulars are reproduced above exactly as you entered',
    ' them and nothing has been lost. The usual cause is too many',
    ' requests in the same minute — wait a minute and generate again to',
    ' get the full court-formatted draft.]',
    '',
