...
const FREE_MODELS = [
  ...
  'llama-3.3-70b-versatile',  // last-resort if 8b instant is rate-limited
]
...
function buildCaseBriefMessages(rawText, opts = {}) {
  ...
  // The Groq 70B model has a 128K-token context window, so we can afford
  // ~45K characters here without crowding out the system prompt or output.
  const HEAD = 30_000, TAIL = 15_000
...
export async function generateShortCaseBriefStream(rawText, opts = {}, onDelta) {
  ...
  const { system, user } = buildCaseBriefMessages(rawText, opts)
  const maxTokens   = opts.isPro === false ? 2200 : 4000
  ...
  for (const model of modelList) {
    try {
      ...
      // empty / too-short — try next model
