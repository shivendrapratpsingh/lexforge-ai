...
function buildSystemPrompt(documentType, court, language) {
  ...
  return FIDELITY_MANDATE + base + SUPREME_COURT_STYLE + courtAddendum(court) + languageAddendum(language)
}
...
REMINDER: Any name, date, address, amount, FIR number, cheque number, or other particular not present above must be a bracketed placeholder, NOT invented content.`
  ...
  try {
    ...
    ], 4000, 0.1, { isPro: opts.isPro })  // lower temperature → less invention
