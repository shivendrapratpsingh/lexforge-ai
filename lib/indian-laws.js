...
export const INDIAN_LAWS = [
  ...
  { id:'PMLA', shortName:'PMLA',          fullName:'The Prevention of Money Laundering Act, 2002',                                                                          year:2002, category:'Criminal',    tags:['pmla','money laundering','ed','section 3','section 19','section 45','attachment'], keySections:[
...
export function findApplicableLaws(query, limit = 8) {
  ...
  return _laws.map(l => {
    ...
    for (const tag of l.tags) {
      ...
      else if (tokens.some(t => tag.includes(t))) score += 1
    }
    if (q.includes(l.shortName.toLowerCase())) score += 5
    if (q.includes(l.id.toLowerCase()))         score += 4
    for (const ks of l.keySections) {
      if (q.includes(ks.n.toLowerCase()))       score += 4
    }
    return { law: l, score }
  })
  .filter(x => x.score > 0)
