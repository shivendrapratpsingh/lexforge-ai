...
import { INDIAN_LAWS_EXTRA } from './indian-laws-extra.js'
import { expandLayTerms } from './lay-terms.js'
...
const INDIAN_LAWS_CORE = [
  ...
  { id:'BNS',  shortName:'BNS',  fullName:'The Bharatiya Nyaya Sanhita, 2023',                                                                                          year:2023, category:'Criminal (substantive)', repeals:'IPC 1860', tags:['criminal','offence','punishment','bns','new criminal law','murder','theft','cheating','defamation','reputation'], keySections:[
  ...
  { id:'IPC',  shortName:'IPC',  fullName:'The Indian Penal Code, 1860 (repealed by BNS 2023 w.e.f. 1 July 2024 — still applies to pre-BNS offences)',                  year:1860, category:'Criminal (substantive)', tags:['criminal','offence','punishment','ipc','murder','theft','cheating','420','302','376','498a','defamation','reputation','499','500'], keySections:[
...
export const INDIAN_LAWS = [...INDIAN_LAWS_CORE, ...INDIAN_LAWS_EXTENDED, ...INDIAN_LAWS_EXTRA]
...
export function findApplicableLaws(query, limit = 8) {
  if (!query) return []
  // The corpus is tagged in legal vocabulary; people type plain English.
  // Measured before this: ten of fifteen everyday phrasings — "boss fired
  // me", "husband beats me", "defective product refund" — matched nothing
  // at all. expandLayTerms appends legal terms alongside the user's own
  // words, so precise legal input scores exactly as it did before.
  const q = expandLayTerms(String(query)).toLowerCase()
