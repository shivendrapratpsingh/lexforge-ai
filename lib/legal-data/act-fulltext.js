...
/** Find the official PDF for an Act from its India Code handle page. */
export async function findActPdfUrl(handle) {
  ...
  if (!res.ok) {
    throw Object.assign(new Error(`India Code responded ${res.status}`), { code: 'PROVIDER_ERROR' })
  }
  const html = await res.text()

  // The Act itself is the /bitstream/ PDF. Other links on the page point
  // at rules and notifications, which are separate documents.
  const m = html.match(/href="(\/bitstream\/[^"]+\.pdf)"/i)
  if (!m) {
    throw Object.assign(new Error('No official PDF is published for this Act.'), { code: 'NO_PDF' })
  }
  return {
    pdfUrl: INDIA_CODE_BASE + m[1].replace(/&amp;/g, '&'),
    pageUrl: page,
  }
}
