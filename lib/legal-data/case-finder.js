 ...
 */
export async function findRelevantCases(details, { perSearch = 0 } = {}) {
  ...
  const query = await buildCaseSearchQuery({ ...details, facts })
  ...
  const [apex, general] = await Promise.allSettled([
    searchJudgments({ query, court: 'supremecourt', page: perSearch }),
    searchJudgments({ query, page: perSearch }),
  ])
