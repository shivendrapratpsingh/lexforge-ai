...
//   GET https://webapi.ecourtsindia.com/api/partner/case/DLHC010001232024
//   Authorization: Bearer eci_live_…
// The CNR travels in the path, not a JSON body, and the method is GET.
async function call(path, { method = 'GET', body, query, userId = null } = {}) {
  ...
  // Billed per lookup whatever the outcome, so recorded on failure too.
  trackUsage({ userId, provider: 'ecourts', operation: 'case', ok: res.ok })
...
export async function caseByCnr(cnr, { userId = null } = {}) {
  ...
  return normalise(await call(`/api/partner/case/${encodeURIComponent(clean)}`, { userId }))
}
...
export async function casesByParty({ name, stateCode, districtCode, year }) {
  ...
  const data = await call('/api/partner/case/search', {
    query: { name: String(name).trim(), stateCode, districtCode, year },
  })
