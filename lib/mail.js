 ...
 */
export function planExpiringEmail({ name, planLabel, currentEnd, freeLimit = 2 }) {
  const greeting = name ? `Hello ${name},` : 'Hello,'
  const until = formatDate(currentEnd)
  ...
  const text = [
    ...
    `where they are. You move back to the free plan — ${freeLimit} document${freeLimit === 1 ? '' : 's'} a month.`,
    '', '— LexForge AI',
  ].join('\n')
  ...
  return {
    ...
    html: emailShell({
      ...
      body: [
        ...
        note(`<b>If you do nothing</b>, you keep your account and every document you have written. You simply move back to the free plan — ${freeLimit} document${freeLimit === 1 ? '' : 's'} a month.`, 'good'),
      ].join(''),
    }),
  }
}
