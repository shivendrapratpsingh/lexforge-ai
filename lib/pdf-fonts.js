...
// Paths are built from the working directory rather than resolved
// through require.resolve(). A template-literal require.resolve() is a
// dynamic module request, and Turbopack fails the BUILD on it:
//
//   Module not found: Can't resolve <dynamic>   lib/pdf-fonts.js
//
// Reading the file by path sidesteps module resolution entirely, which
// is what we want anyway — a .ttf is data, not a module. The files are
// placed in the deployment by outputFileTracingIncludes in
// next.config.ts, at exactly this relative path.
const ROOTS = [process.cwd(), join(process.cwd(), '..')]

async function readFromNodeModules(relPath) {
  let lastErr
  for (const root of ROOTS) {
    try {
      return await readFile(join(root, 'node_modules', relPath))
    } catch (e) { lastErr = e }
  }
  throw lastErr
}
...
async function faceBase64(script, weight) {
  ...
  const buf = await readFromNodeModules(`${script.pkg}/${script.faces[weight]}`)
