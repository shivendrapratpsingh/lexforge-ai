...
export const PROCESSORS = [
  // The AI provider is configurable (AI_PROVIDERS in lib/ai.js), and a
  // college is entitled to know who actually sees the text — so BOTH
  // possible providers are named rather than only the one currently
  // first in the chain. Naming only the primary would make this table
  // wrong the moment the chain fell through to the other, which is
  // exactly when it matters.
  {
    name: 'Google (Gemini API)',
    country: 'United States',
    short: 'receives the facts you enter, to generate the draft. This is the primary AI provider and sees your document text.',
  ...
  },
  {
    name: 'Groq',
    country: 'United States',
    short: 'is the reserve AI provider, used only when the primary one is unavailable. It sees the same document text when it is used.',
    purpose: 'Generates the draft when the primary provider fails.',
    data: 'The text of the document being drafted.',
  },
  {
    name: 'Neon',
