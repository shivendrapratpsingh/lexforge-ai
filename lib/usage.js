...
export const RATES = {
  ...
  //
  // Gemini costs more per token than gpt-oss and the cost model must not
  // keep reporting gpt-oss numbers after the provider moved. Blended at
  // the ratio this app actually sends — roughly two-thirds prompt, one
  // third completion — gemini-3.8-flash works out near ₹1.77 per million
  // tokens, i.e. ~16 paise per 1K. Rounded up, same as above.
  //
  //   gpt-oss-120b        $0.15 in / $0.60 out  ->  ~3 paise / 1K
  //   gemini-3.8-flash    $0.75 in / $3.75 out  ->  ~16 paise / 1K
  //   gemini-2.5-flash-lite $0.10 in / $0.40 out -> ~2 paise / 1K
  //
  // Set RATE_AI_PER_KTOKEN to override without a deploy.
  geminiPerKToken: num(process.env.RATE_AI_PER_KTOKEN, 16),  // 16 paise / 1K tokens

  // OpenRouter is now the head of the chain, and without a rate of its
  // own every call it served was costed at ZERO. The panel would have
  // reported that AI is free and invited a price set below cost — the
  // exact failure the fixedMonthlyPaise note above warns about, arriving
  // through a different door.
  //
  // It bills pass-through at the underlying model's price plus a 5.5%
  // fee taken when credits are bought, not per call. Defaulting to the
  // Gemini rate: OpenRouter's Pro chain routes to gemini-3.7-flash, and
  // where the routing lands on 3.5-flash-lite the real cost is lower —
  // overstating, as every other rate here does deliberately.
  openrouterPerKToken: num(process.env.RATE_OPENROUTER_PER_KTOKEN,
                           num(process.env.RATE_AI_PER_KTOKEN, 16)),
  ...
  kanoonDoc:     num(process.env.RATE_KANOON_DOC, 20),       // ₹0.20 — /doc/
  // The cheap endpoints, and they were being costed as /doc/ because
  // '/docfragment' and '/docmeta' both satisfy startsWith('/doc'). A
  // fragment was billed at four times its real price, which matters now
  // that treatment checking makes many of them per judgment.
  kanoonFragment: num(process.env.RATE_KANOON_FRAGMENT, 5),  // ₹0.05
  kanoonMeta:     num(process.env.RATE_KANOON_META, 2),      // ₹0.02
  kanoonOrigDoc:  num(process.env.RATE_KANOON_ORIGDOC, 50),  // ₹0.50 — /origdoc/
...
export function estimateCost({ provider, operation, tokens = 0, calls = 1 }) {
  switch (provider) {
    ...
    case 'gemini':
      return Math.ceil((tokens / 1000) * RATES.geminiPerKToken)
    case 'openrouter':
      return Math.ceil((tokens / 1000) * RATES.openrouterPerKToken)
    case 'indiankanoon':
      return calls * ({
        doc:      RATES.kanoonDoc,
        fragment: RATES.kanoonFragment,
        meta:     RATES.kanoonMeta,
        origdoc:  RATES.kanoonOrigDoc,
      }[operation] ?? RATES.kanoonSearch)
    case 'ecourts':
...
function breakEvenAt(perUserVariablePaise) {
  ...
  const out = {}
  for (const price of [200, 251, 400, 600]) {   // college seat, direct, and two what-ifs
