...
export const RATES = {
  ...
  // Indian Kanoon bills per call. ₹500 of credit is the free grant.
  // Set the real figure once a week of usage shows what it actually is.
  kanoonSearch:  num(process.env.RATE_KANOON_SEARCH, 200),   // ₹2.00
  kanoonDoc:     num(process.env.RATE_KANOON_DOC, 300),      // ₹3.00
...
export async function usageSummary({ days = 30, institutionId = null } = {}) {
  ...
  return {
    ...
    activeUsers: users,
    // The headline number: what one active user costs per month. A price
    // below this loses money on every single user.
